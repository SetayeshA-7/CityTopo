  ...
  if (challengeInfo) {
    ...
    if (challengeStatus) {
      challengeStatus.innerHTML = `
        ...
        <button onclick="showChallengeHint(${challengeId})" class="btn btn-sm btn-info">
          <i class="fas fa-lightbulb"></i> Hint
        </button>
      `;
    }
...
function checkCurrentChallenge() {
  ...
  if (result.completed) {
    ...
    // Update challenge status with error message
    const challengeStatus = document.getElementById('currentChallengeStatus');
    if (challengeStatus) {
      // Only update if we haven't shown success message before
      if (!challengeStatus.innerHTML.includes("Challenge Completed")) {
        const challengeInfo = cityGraph.startChallenge(currentChallenge);
        if (challengeInfo) {
          challengeStatus.innerHTML = `
            <p><strong>${challengeInfo.title}</strong></p>
            <p>${challengeInfo.description}</p>
            ...
            <button onclick="showChallengeHint(${currentChallenge})" class="btn btn-sm btn-info">
              <i class="fas fa-lightbulb"></i> Hint
            </button>
          `;
        }
